...
export default {
  ...
  methods: {
    handleGetDays(year, month, startOfWeek) {
      this.showDays = []
      this.days = getDaysInMonth(year, month) // 获取本月的总天数
      const firstDayOfWeek = new Date(year, month, 1).getDay() // 获取本月第一天是周几 [0, 6]
      ...
      console.log(`本月第一天周${firstDayOfWeek}`, index)
    ...
    handleItemClick(item, i, j) {
      ...
      if (this.selectedMode) {
        ...
        if (this.moveIndex.length === 2) { // 数组长度为2 表示已经选择了一个区间，要进行重置，再选！
